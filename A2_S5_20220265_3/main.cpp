...
vector<string>split(string target, string delimiter){
    ...
    for(int i=0 ; i<target.size(); i++){
        ...
        if(target[i]==delimiter[0]) {
            ...
            //a loop that will save the possible existence of the dictmaker in the string
            for (int j = 0; j < delimiter.size() ; j++) {
                test += target[i + j];
            }
            if (delimiter == test) {
                // we add to i the size of the delimiter string and subtracting 1 because the for loop adds 1,
...
int main() {
    ...
    vector<string>s2 = split("myspacelaptopspaceplaysspaceclion", "space");
